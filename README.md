...
- `container-job` provides a local container-engine execution skeleton with task-file injection and JSON result capture
...
- A2A bridge traffic now normalizes around `message_envelope` and `message_result` schemas for the same reason
...
- when `operator_identities` are configured, those governance state reads now require a signed `read-only` request or a higher-privilege signed operator scope
