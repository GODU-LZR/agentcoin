...
- operator auth is still not production-complete, but Tier 2 workflow-admin and bridge-admin plus Tier 3 and Tier 4 write paths now support signed operator requests, nonce replay rejection, denial policy receipts, durable auth audit records, and workflow / bridge governance receipts; read-only scopes, scoped bearer tokens, and broader deployment hardening remain unfinished
...
3. Extend the new signed operator auth path into read-only scopes, scoped bearer tokens, stronger secret handling, and outbound ACL policy
