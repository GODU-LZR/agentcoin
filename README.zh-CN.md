...
<p align="center">
  ...
  ·
  <a href="docs/architecture/poaw-settlement-policy.md">PoAW 策略</a>
  ·
  <a href="docs/architecture/dispatch-scoring.md">调度评分</a>
  ·
...
- `非对称身份`：节点现在也可以用兼容 `ssh-keygen` 的 `Ed25519` 密钥为 card、任务和回执签名，peer 也可以在轮换期间预置信任替换公钥，operator 还可以导出带严重度排序的 trust reconciliation 结果，并查看建议动作及 runtime 或 config diff 预览。
...
- `container-job` 提供本地容器引擎执行骨架，支持任务文件注入和 JSON 结果回收
...
- `AgentCard.runtime_capabilities` 现在会对外广播 runtime 的结构化能力，例如 structured output 和 JSON Schema 支持
...
- `GET /v1/schema/capabilities` 可返回能力族、别名和隐含关系
- `GET /v1/tasks/dispatch/preview` 可返回带语义评分的 dispatch 候选列表
- `POST /v1/tasks/dispatch/evaluate` 可按完整任务评估目标节点，包含 `_runtime` 和 `_bridge` 约束
...
- 结算判定层现在也有单独文档：`docs/architecture/poaw-settlement-policy.md`
...
- `POST /v1/onchain/settlement-relay` 会顺序提交 raw transaction 并返回签名 relay receipt
...
- 当配置了 `operator_identities` 时，这些治理状态读取现在要求 signed `read-only` 请求，或更高权限的已签名运维 scope
