...
- `非対称 identity`: `ssh-keygen` 互換の `Ed25519` key で card, task, receipt を署名でき、rotation 期間は peer 側で置き換え key を事前信頼でき、operator は severity-ranked な trust reconciliation export と suggested action、runtime / config diff preview も取得できます
...
- `container-job` は task-file 注入と JSON result capture を備えた local container-engine execution skeleton を提供します
...
- `AgentCard.runtime_capabilities` で structured output や JSON Schema support のような runtime feature を機械可読で広告します
...
- `A2A` bridge は `message_envelope` と `message_result` schema に正規化され、message import / execution / export で同じ構造を共有します
...
- `operator_identities` を設定すると、これらの governance state read は signed `read-only` request、またはそれを包含する高権限の signed operator scope を要求します
