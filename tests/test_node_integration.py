...
class NodeIntegrationTests(unittest.TestCase):
    ...
    def test_workflow_execute_accepts_service_scoped_renter_token(self) -> None:
        ...
        try:
            ...
            self.assertEqual(renter_token["scope"]["workflow_name"], "premium-review")
            self.assertEqual(renter_token["scope"]["service_id"], "premium-review")
            self.assertEqual(renter_token["scope"]["allowed_operations"], ["workflow-execute"])
            ...
            self.assertEqual(token_introspect_before["introspection"]["scope"]["allowed_operations"], ["workflow-execute"])
    ...
    def test_renter_token_scope_rejects_wrong_service_or_operation(self) -> None:
        key_path, public_key = self._generate_identity(Path(self.tempdir.name) / "id_client_renter_scope", "frontend-local-renter-scope")
        onchain = OnchainBindings(
            enabled=True,
            chain_id=97,
            rpc_url="https://bsc-testnet.example/rpc",
        ...
        node = NodeHarness(
            node_id="payment-renter-scope-node",
            token="token-payment-renter-scope",
            db_path=str(Path(self.tempdir.name) / "payment-renter-scope.db"),
            capabilities=["worker"],
            signing_secret="payment-renter-scope-secret",
            payment_required_workflows=["premium-review"],
            onchain=onchain,
        )
        node.start()
        try:
            challenge_status, challenge_payload = self._identity_signed_post(
                ...
                private_key_path=key_path,
                principal="frontend-local-renter-scope",
                public_key=public_key,
            )
            self.assertEqual(challenge_status, HTTPStatus.PAYMENT_REQUIRED)
            challenge_id = challenge_payload["payment"]["challenge"]["challenge_id"]
            ...
            wrong_service_status, wrong_service_payload = self._identity_signed_post(
                f"{node.base_url}/v1/payments/renter-tokens/introspect",
                {
                    "workflow_name": "premium-review",
            ...
            self.assertEqual(wrong_service_status, HTTPStatus.BAD_REQUEST)
            self.assertIn("service", wrong_service_payload["error"])

            wrong_operation_status, wrong_operation_payload = self._identity_signed_post(
                f"{node.base_url}/v1/payments/renter-tokens/introspect",
                {
                    "workflow_name": "premium-review",
                    "service_id": "premium-review",
    ...
    def test_renter_token_status_and_summary_reflect_usage(self) -> None:
        key_path, public_key = self._generate_identity(Path(self.tempdir.name) / "id_client_renter_summary", "frontend-local-renter-summary")
        onchain = OnchainBindings(
            enabled=True,
            chain_id=97,
            rpc_url="https://bsc-testnet.example/rpc",
        ...
        node = NodeHarness(
            node_id="payment-renter-summary-node",
            token="token-payment-renter-summary",
            db_path=str(Path(self.tempdir.name) / "payment-renter-summary.db"),
            capabilities=["worker"],
            signing_secret="payment-renter-summary-secret",
            payment_required_workflows=["premium-review"],
            services=[
                ServiceCapabilityConfig(
                    service_id="premium-review",
        ...
        try:
            ...
            issue_status, issued = self._post(
                f"{node.base_url}/v1/payments/receipts/issue",
                "token-payment-renter-summary",
                {
                    "challenge_id": challenge_id,
                    "payer": "did:agentcoin:ssh-ed25519:renterpayer",
                    "tx_hash": "0xsummary123",
                },
            )
            self.assertEqual(issue_status, HTTPStatus.CREATED)
            receipt = issued["receipt"]
            ...
            status_before, token_before = self._identity_signed_get(
                f"{node.base_url}/v1/payments/renter-tokens/status?token_id={renter_token['token_id']}",
                private_key_path=key_path,
                principal="frontend-local-renter-summary",
                public_key=public_key,
            )
            self.assertEqual(status_before, HTTPStatus.OK)
            self.assertEqual(token_before["token"]["remaining_uses"], 2)
            self.assertEqual(token_before["token"]["usage_count"], 0)
            ...
            status_after, token_after = self._identity_signed_get(
                f"{node.base_url}/v1/payments/renter-tokens/status?token_id={renter_token['token_id']}",
                private_key_path=key_path,
                principal="frontend-local-renter-summary",
                public_key=public_key,
            )
            self.assertEqual(status_after, HTTPStatus.OK)
            self.assertEqual(token_after["token"]["remaining_uses"], 1)
            self.assertEqual(token_after["token"]["usage_count"], 1)
            self.assertEqual(token_after["token"]["status"], "issued")

            summary_status, summary = self._identity_signed_get(
                f"{node.base_url}/v1/payments/renter-tokens/summary?workflow_name=premium-review&service_id=premium-review&limit=5",
                private_key_path=key_path,
                principal="frontend-local-renter-summary",
                public_key=public_key,
            )
            self.assertEqual(summary_status, HTTPStatus.OK)
            self.assertEqual(summary["workflow_name"], "premium-review")
            self.assertEqual(summary["service_id"], "premium-review")
            self.assertEqual(summary["item_count"], 1)
            self.assertEqual(summary["active_count"], 1)
            self.assertEqual(summary["counts"]["issued"], 1)
            self.assertEqual(summary["total_remaining_uses"], 1)
    ...
    def test_runtime_adapter_claude_http_follow_up_turn_preserves_assistant_tool_use_and_tool_result(self) -> None:
        gateway = ClaudeHttpHarness()
        gateway.start()
        node = NodeHarness(
            node_id="runtime-claude-http-follow-up-node",
            token="token-claude-http-follow-up",
            db_path=str(Path(self.tempdir.name) / "runtime-claude-http-follow-up.db"),
            capabilities=["worker"],
            runtimes=["claude-http"],
        )
        node.start()
        try:
            self._post(
                f"{node.base_url}/v1/tasks",
                "token-claude-http-follow-up",
                {
                    "id": "runtime-claude-http-follow-up-1",
                    "kind": "generic",
                    "role": "worker",
                    "payload": {"input": {"prompt": "continue after external tool execution"}},
                },
            )
            bind_status, bound = self._post(
                f"{node.base_url}/v1/integrations/claude-http/bind",
                "token-claude-http-follow-up",
                {
                    "task_id": "runtime-claude-http-follow-up-1",
                    "endpoint": gateway.url,
                    "model": "claude-3-7-sonnet-latest",
                    "auth_token": "anthropic-secret",
                    "prompt": "continue after external tool execution",
                    "assistant_tool_uses": [
                        {
                            "id": "toolu_prior_2",
                            "name": "repo_status",
            ...
            worker = WorkerLoop(
                node_url=node.base_url,
                token="token-claude-http-follow-up",
                worker_id="worker-claude-http-follow-up-1",
                capabilities=["worker"],
                lease_seconds=30,
                adapter_policy=AdapterPolicy(
                    allowed_runtime_kinds=["claude-http"],
            ...
            self.assertEqual(len(gateway.calls[0]["messages"]), 3)
            self.assertEqual(gateway.calls[0]["messages"][1]["role"], "assistant")
            self.assertEqual(gateway.calls[0]["messages"][1]["content"][0]["type"], "tool_use")
            self.assertEqual(gateway.calls[0]["messages"][1]["content"][0]["id"], "toolu_prior_2")
            self.assertEqual(gateway.calls[0]["messages"][2]["role"], "user")
            self.assertEqual(gateway.calls[0]["messages"][2]["content"][0]["type"], "tool_result")
            self.assertEqual(gateway.calls[0]["messages"][2]["content"][0]["tool_use_id"], "toolu_prior_2")

            _, tasks = self._get(f"{node.base_url}/v1/tasks")
            task = [item for item in tasks["items"] if item["id"] == "runtime-claude-http-follow-up-1"][0]
            self.assertEqual(task["result"]["adapter"]["protocol"], "claude-http")
            self.assertEqual(
                task["result"]["runtime_execution"]["follow_up_turn"]["assistant_tool_use_ids"],
                ["toolu_prior_2"],
            )
            self.assertEqual(
                task["result"]["runtime_execution"]["follow_up_turn"]["tool_result_ids"],
                ["toolu_prior_2"],
            )
            self.assertEqual(
                task["result"]["execution_receipt"]["artifacts"]["assistant_tool_uses"][0]["name"],
                "repo_status",
            )
        finally:
            node.stop()
    ...
    def test_claude_http_follow_up_bind_reuses_source_tool_use_for_target_task(self) -> None:
        gateway = ClaudeHttpHarness()
        gateway.start()
        node = NodeHarness(
            node_id="runtime-claude-http-follow-up-bind-node",
            token="token-claude-http-follow-up-bind",
            db_path=str(Path(self.tempdir.name) / "runtime-claude-http-follow-up-bind.db"),
            capabilities=["worker"],
            runtimes=["claude-http"],
        )
        node.start()
        try:
            self._post(
                f"{node.base_url}/v1/tasks",
                "token-claude-http-follow-up-bind",
                {
                    "id": "runtime-claude-http-source-1",
                    "kind": "generic",
                    "role": "worker",
                    "payload": {"input": {"prompt": "lookup repository status"}},
                },
            ...
            self._post(
                ...
                "token-claude-http-follow-up-bind",
                {
                    "id": "runtime-claude-http-target-1",
                    "kind": "generic",
                    "role": "worker",
                    "payload": {"input": {"prompt": "continue with result"}},
                },
            )
            self._post(
                f"{node.base_url}/v1/integrations/claude-http/bind",
                "token-claude-http-follow-up-bind",
                {
                    "task_id": "runtime-claude-http-source-1",
                    "endpoint": gateway.url,
                    "model": "claude-3-7-sonnet-latest",
                    "auth_token": "anthropic-secret",
                    "prompt": "lookup repository status",
            ...
            worker = WorkerLoop(
                node_url=node.base_url,
                token="token-claude-http-follow-up-bind",
                worker_id="worker-claude-http-follow-up-bind-1",
                capabilities=["worker"],
                lease_seconds=30,
                adapter_policy=AdapterPolicy(
                    allowed_runtime_kinds=["claude-http"],
            ...
            bind_status, bound = self._post(
                f"{node.base_url}/v1/integrations/claude-http/follow-up-bind",
                "token-claude-http-follow-up-bind",
                {
                    "task_id": "runtime-claude-http-target-1",
                    "source_task_id": "runtime-claude-http-source-1",
            ...
            self.assertEqual(gateway.calls[1]["messages"][0]["role"], "user")
            self.assertEqual(gateway.calls[1]["messages"][0]["content"][0]["text"], "continue with result")
            self.assertEqual(gateway.calls[1]["messages"][1]["role"], "assistant")
            self.assertEqual(gateway.calls[1]["messages"][1]["content"][0]["type"], "tool_use")
            self.assertEqual(gateway.calls[1]["messages"][1]["content"][0]["id"], "toolu_1")
            self.assertEqual(gateway.calls[1]["messages"][2]["role"], "user")
            self.assertEqual(gateway.calls[1]["messages"][2]["content"][0]["type"], "tool_result")
            self.assertEqual(gateway.calls[1]["messages"][2]["content"][0]["tool_use_id"], "toolu_1")

            _, tasks = self._get(f"{node.base_url}/v1/tasks")
            target_task = [item for item in tasks["items"] if item["id"] == "runtime-claude-http-target-1"][0]
            self.assertEqual(
                target_task["result"]["runtime_execution"]["follow_up_turn"]["assistant_tool_use_ids"],
                ["toolu_1"],
            )
            self.assertEqual(
